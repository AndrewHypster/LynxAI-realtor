...
const find = async ({ sheets, range, element }) => {
  ...
  // 2. Читаємо другу колонку (крім заголовка)
...
export async function POST(req) {
  ...
  try {
    ...
    if (body.action === "ADD_USER" && body.email && body.password) {
      const user = await find({ sheets, range: "admins!", element: body.email });

      console.log('api sheets', user);
      ...
      const res = sheets.spreadsheets.values.append({
        ...
        requestBody: {
          values: [[body.name, body.email, body.image, body.password]],
        },
      });
